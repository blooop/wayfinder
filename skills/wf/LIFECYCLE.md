...
4. **Gate failure**: hand **one** fresh retry subagent the failing run's own output **verbatim, with its URL beside it** — a quotation of a read the manager just made, which is the only kind of thing it may pass on and the reason it must not be paraphrased. A second failure parks the node — a `### handoff` comment on its ticket with the failure attached — and the run moves on to other work. Before chasing a failure, check whether the same job fails on the default branch: a pre-existing or infrastructure failure is recorded, not fixed.
