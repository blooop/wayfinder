    ...
    let mut sources: Vec<(String, String)> = std::fs::read_dir(&tests)
        ...
        .filter_map(|path| {
            ...
            (is_rust && name.starts_with("live_")).then(|| {
                let text = std::fs::read_to_string(tests.join(&name))
    ...
    for (name, text) in &live_sources() {
        ...
        assert_eq!(
            ...
            "{name} contains {written} test attributes but the scanner in this \
             file paired only {} of them with a function. The difference is \
             written in a shape it cannot read — an attribute split across \
